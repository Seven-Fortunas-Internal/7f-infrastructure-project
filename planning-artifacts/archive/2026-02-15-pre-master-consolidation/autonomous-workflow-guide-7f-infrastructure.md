...
3. **GitHub CLI authenticated as jorge-at-sf (CRITICAL)**:
```bash
# IMPORTANT: Must authenticate as jorge-at-sf, NOT jorge-at-gd
gh auth login
gh auth status | grep jorge-at-sf

# If authenticated as wrong account, switch:
gh auth switch --user jorge-at-sf
gh auth status | grep jorge-at-sf  # Verify correct account
```

**Why jorge-at-sf is critical:** All GitHub organization operations must be performed under the jorge-at-sf account to ensure proper ownership and access control. Using the wrong account (jorge-at-gd) will create orgs under incorrect ownership.
