 ...
 *
 * The tests are run via Vitest from dashboards/ai/ (vite.config.js includes this path).
 * Run: cd dashboards/ai && npx vitest run ../../tests/component/dashboard/
 *
 * Scenarios tested:
...
import App from '../../../dashboards/ai/src/App'
...
// ---------------------------------------------------------------------------
// S2: Fetch error — cached_updates.json fails
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// S5: Successful load — renders update cards
// ---------------------------------------------------------------------------
...
describe('S5 — Successful data load', () => {
  it('renders one article card per update', async () => {
    ...
    await waitFor(() => {
      ...
      expect(cards).toHaveLength(MOCK_UPDATES.length)
    })
  })

  it('renders update titles as link text', async () => {
    mockFetch({ updates: MOCK_UPDATES })
    render(<App />)
    await waitFor(() => {
...
describe('S6 — Search filter', () => {
  ...
  it('filters updates by summary text', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Gemini Ultra 2.0 Benchmark Results')).toBeInTheDocument()
    })
  })
